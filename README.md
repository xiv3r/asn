...
* It will perform **IP geolocation** lookups according to the logic described [below](#geolocation).
  ...
  * the script can also **map all IPv4/IPv6 CIDR blocks** allocated to any given country, by querying data from IPdeny's [ipblocks](https://www.ipdeny.com/ipblocks/) resource, which is kept updated directly from regional registries (RIR) IP block allocations. See [below](#mapping-the-ipv4v6-address-space-of-specific-countries) for more info.
